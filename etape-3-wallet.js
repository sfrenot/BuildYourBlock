const Wallet = require("./Wallet")

const PUBLIC_KEY_A = `-----BEGIN PUBLIC KEY-----

mQIDAQAB
-----END PUBLIC KEY-----`;

const PRIVATE_KEY_A = `-----BEGIN RSA PRIVATE KEY-----

ptK3iCr990g4yqQyWNDvXqrdz3BrhsLJrdwYooX+iU2VHxj+dKKP
-----END RSA PRIVATE KEY-----`;

const PUBLIC_KEY_B = `-----BEGIN PUBLIC KEY-----

SQIDAQAB
-----END PUBLIC KEY-----`;

const PRIVATE_KEY_B = `-----BEGIN RSA PRIVATE KEY-----

-----END RSA PRIVATE KEY-----`;

const walletA = new Wallet(PRIVATE_KEY_A);
const walletB = new Wallet(PRIVATE_KEY_B);
const walletAPublic = new Wallet(PUBLIC_KEY_A);

console.log("Je dois avoir la clé privée de A :", walletA.hasPrivate());
console.log("Pour B aussi :", walletA.hasPrivate());
console.log("Mais pas pour A Public :", walletAPublic.hasPrivate());

console.log("Et la clé publique de A doit être la bonne :", walletA.getPublicKey() === PUBLIC_KEY_A);
console.log("Et celle de A Public aussi :", walletAPublic.getPublicKey() === PUBLIC_KEY_A);

const text = 'Hello RSA!';

const signatureA = walletA.sign(text);
console.log('Signature A : ', signatureA);
const verifyA = walletA.verify(text, signatureA);
console.log('Verify A : ', verifyA);

const signatureB = walletB.sign(text);
console.log('Signature B : ', signatureB);
const verifyB = walletB.verify(text, signatureB);
console.log('Verify B : ', verifyB);

const verifyAByAPublic = walletAPublic.verify(text, signatureA);
console.log('verify A by A public : ', verifyAByAPublic);
